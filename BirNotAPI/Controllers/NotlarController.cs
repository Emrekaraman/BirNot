...
namespace BirNotAPI.Controllers
{
    ...
    public class NotlarController : ApiController
    {
        ...
        {
            return db.Notlar
                .Where(x => x.YazarId == UserId).Select(x => new GetNotDTO()
                {
                    Id = x.Id,
                    Baslik = x.Baslik,
                    Icerik = x.Icerik,
        ...
        public IHttpActionResult PostNot(YeniNotDTO notDTO)
        {
            ...
            return CreatedAtRoute("DefaultApi", new { id = not.Id }, new GetNotDTO()
            {
                Id = not.Id,
                Baslik = not.Baslik,
                Icerik = not.Icerik,
